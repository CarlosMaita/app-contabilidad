            ...
            <div class="panel flex flex-col gap-4 p-4 sm:p-6">
                <div class="flex flex-wrap items-end justify-between gap-4">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('exports.balance', ['format' => 'pdf', 'as_of' => $asOf]) }}" class="btn-secondary">PDF</a>
                        <a href="{{ route('exports.balance', ['format' => 'xlsx', 'as_of' => $asOf]) }}" class="btn-secondary">XLSX</a>
                    </div>
                </div>
        ...
        @if ($tab === 'pnl' && $pnl)
            <div class="panel flex flex-col gap-4 p-4 sm:p-6">
                <div class="flex flex-wrap items-end justify-between gap-4">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('exports.pnl', ['format' => 'pdf', 'from' => $from, 'to' => $to]) }}" class="btn-secondary">PDF</a>
                        <a href="{{ route('exports.pnl', ['format' => 'xlsx', 'from' => $from, 'to' => $to]) }}" class="btn-secondary">XLSX</a>
                    </div>
                </div>

                @foreach (['income' => $pnl['income'], 'expense' => $pnl['expense']] as $key => $section)
                    <div>
                        <h3 class="mb-2 border-b-2 border-ink pb-1 text-sm font-extrabold uppercase tracking-[0.06em]">{{ $key === 'income' ? 'Ingresos' : 'Gastos' }}</h3>
                        <table class="w-full text-[13px]">
                            <tbody>
                                ...
                                @empty
                                    <tr><td class="py-1 text-neutral-700">Sin movimientos.</td><td></td></tr>
                                @endforelse
                                <tr>
                                    <td class="border-t-2 border-ink py-1.5 font-extrabold">Total {{ $key === 'income' ? 'Ingresos' : 'Gastos' }}</td>
                                    <td class="border-t-2 border-ink py-1.5 text-right font-mono font-extrabold">{{ number_format((float) $section['total'], 2, ',', '.') }}</td>
                                </tr>
                            </tbody>
                        </table>
                ...
                <div class="border px-4 py-3 text-[13px] font-extrabold {{ (float) $pnl['result'] >= 0 ? 'border-ink' : 'border-accent text-accent-600' }}">
